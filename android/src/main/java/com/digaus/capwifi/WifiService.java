...
public class WifiService {
	...
	public void getConnectedSSID(PluginCall call) {
		...
		call.error("FAILED_TO_GET_CONNECTED_NETWORK");
	}
	...
	private int ssidToNetworkId(String ssid, String authType) {
		try {
			...
			for (WifiConfiguration network : currentNetworks) {
				Log.i(TAG, "Network: " + network.SSID + "|" + getSecurityType(network));
				...
				if (network.SSID != null) {
					if (authType.length() == 0) {
						...
						String testSSID = network.SSID + getSecurityType(network);
			...
			if (networkId == -1 && authType.substring(0, 3).equals("WPA")) {
				for (WifiConfiguration network : currentNetworks) {
					if (network.SSID != null) {
						String testSSID = network.SSID + getSecurityType(network).substring(0, 3);
						if (testSSID.equals(ssid + authType)) {
							networkId = network.networkId;
						}
					}
			...
			if (useWifi) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						// if not connected till 1 min release and reject
						if (WifiService.this.savedCall != null && !WifiService.this.savedCall.isReleased()) {
							forceWifiUsageQ(false, null);
							WifiService.this.savedCall.error("CONNECTION_FAILED");
							WifiService.this.savedCall = null;
						}
					}
				}).start();
	...
	private static class ConnectAsync extends AsyncTask<Object, Void, String[]> {
		PluginCall call;
		...
		protected String[] doInBackground(Object... params) {
			...
			for (int i = 0; i < TIMES_TO_RETRY; i++) {
				...
				WifiInfo info = wifiService.wifiManager.getConnectionInfo();
